...
class MethodGetModemInfo(ApiMethod):
  ...
  async def _get_modem_info_over_at(self) -> dict | None:
    try:
      response = await self._api_client.exec("sms.modem_info", {})
      if response.is_ok():
        return response.get_data()
      self._logger.warning(f"sms.modem_info error: {response.get_error()}")
    except Exception as e:
      self._logger.warning(f"sms.modem_info call failed: {e}")
    return None
