...
class MethodSendText(ApiMethod):
  ...
  async def exec(self, args):
    ...
    asyncio.create_task(self._do_send_message(chat_id, text, reply_markup))

    return {"status": "ok"}

  async def _do_send_message(self, chat_id, text, reply_markup):
    try:
      with self._plugin.keep_alive.wait_until_done(
        const.KEEP_ALIVE_WAIT_FINISH_OUTCOME_REQUEST_KEY, const.TIMEOUT_SEND_MESSAGE
      ):
        await self._tg_api.send_message(chat_id, text, reply_markup=reply_markup)
