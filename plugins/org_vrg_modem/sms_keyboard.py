...
async def get_sms_keyboard(
  ...
  for sms in all_sms[offset : offset + per_page]:
    ...
    buttons.append(
      [{"text": sms_filename, "callback_data": f"command__modem__get_sms__{sms_filename}"}]
    )
    i += 1
      ...
      buttons_row.append(
        {
          ...
          "callback_data": f"command__modem__list_sms__{x5_page}",
        }
      )

    buttons_row.append(
      {"text": "⬅️ Earlier", "callback_data": f"command__modem__list_sms__{next_page}"}
    )
  if page > 1:
    prev_page = page - 1
    buttons_row.append({"text": "Later ➡️", "callback_data": f"command__modem__list_sms__{prev_page}"})
