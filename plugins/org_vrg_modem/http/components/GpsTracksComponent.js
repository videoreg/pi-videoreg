  ...
  template: `
    <div>
      <div class="page-header">
        <h1 class="page-title">{{ $t('gps.gps_tracks.title') }}</h1>
  ...
  methods: {
    async load() {
      ...
      try {
        ...
        if (!response.ok) {
          this.error = data.error || this.$t('gps.gps_tracks.error_load');
          return;
        }
    ...
    async deleteTrack(track) {
      if (!confirm(this.$t('gps.gps_tracks.confirm_delete', { time: track.time }))) return;
      try {
        const response = await fetch('/api/gps/delete_track', {
          method: 'POST',
        ...
        if (!response.ok) {
          const data = await response.json();
          this.error = data.error || this.$t('gps.gps_tracks.error_delete');
          return;
        }
