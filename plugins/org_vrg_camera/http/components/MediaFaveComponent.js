const MediaFaveComponent = {
  ...
  template: `
    <div>
      <div class="page-header">
        <h1 class="page-title">{{ $t('camera.media_fave.title') }}</h1>
  ...
  methods: {
    ...
    async load() {
      ...
      this.currentPage = 0;
      try {
        const response = await fetch('/api/camera/fave_list?fave=true&all=true', { credentials: 'same-origin' });
        const data = await response.json();
        if (!response.ok) {
          this.error = data.error || this.$t('camera.media_fave.error_load');
          return;
        }
