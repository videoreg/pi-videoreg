...
class MethodSendVideo(ApiMethod):
  _plugin: BotPlugin
  _bot: Bot
  _tg_api: TelegramApi
  ...
  async def _do_send(self, chat_id, file_path, width, height, fallback_message):
    try:
      with self._plugin.keep_alive.wait_until_done(
        const.KEEP_ALIVE_WAIT_FINISH_OUTCOME_REQUEST_KEY, const.TIMEOUT_SEND_VIDEO
      ):
        await self._tg_api.send_chat_action(chat_id, "upload_video")
        result = await self._tg_api.send_video(chat_id, str(file_path), width, height)
