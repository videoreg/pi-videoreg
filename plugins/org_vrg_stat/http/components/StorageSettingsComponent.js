  ...
  template: `
    <div>
      <div class="page-header">
        ...
        <h1 class="page-title">{{ $t('stat.storage.title') }}</h1>
      ...
      <div v-if="partitions.length > 0 || !loading" class="info-block">
        ...
        <div v-if="partitions.length > 0" style="overflow-x: auto;">
          <table class="info-table">
            <thead>
              <tr>
                <th>{{ $t('stat.storage.col_device') }}</th>
                <th>{{ $t('stat.storage.col_mountpoint') }}</th>
                <th>{{ $t('stat.storage.col_fs') }}</th>
                <th style="text-align: right;">{{ $t('stat.storage.col_size') }}</th>
                <th style="text-align: right;">{{ $t('stat.storage.col_used') }}</th>
                <th style="text-align: right;">{{ $t('stat.storage.col_free') }}</th>
                <th style="min-width: 140px;">{{ $t('stat.storage.col_fill') }}</th>
              </tr>
            </thead>
            <tbody>
  ...
  methods: {
    formatBytes(bytes) {
      ...
      if (bytes >= 1024 * 1024 * 1024) {
        return (bytes / (1024 * 1024 * 1024)).toFixed(1) + ' ' + this.$t('stat.storage.unit_gb');
      }
      if (bytes >= 1024 * 1024) {
        return (bytes / (1024 * 1024)).toFixed(0) + ' ' + this.$t('stat.storage.unit_mb');
      }
      if (bytes >= 1024) {
        return (bytes / 1024).toFixed(0) + ' ' + this.$t('stat.storage.unit_kb');
      }
      return bytes + ' ' + this.$t('stat.storage.unit_b');
    },
    ...
    async load() {
      ...
      try {
        ...
        if (!response.ok) {
          this.error = data.error || this.$t('stat.storage.error_load');
          return;
        }
