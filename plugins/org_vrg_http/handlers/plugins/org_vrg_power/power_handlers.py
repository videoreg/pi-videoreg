...
async def handle_get_power_capabilities(request: web.Request):
  ...
  try:
    response = await api_client.exec("power.get_capabilities", {})
    if not response.is_ok():
      return web.json_response({"error": response.get_error()}, status=500)
    return web.json_response(response.get_data())
  except RequestTimeoutError:
    ...
    logger.error(f"Error in handle_get_power_capabilities: {e}", exc_info=True)
    return web.json_response({"error": str(e)}, status=500)
