...
HTTP_LOG_LEVEL="INFO"  # Default value
ENV=""
    ...
    case $1 in
        ...
        --http-log-level)
            if [[ "$2" =~ ^(DEBUG|INFO|WARNING|ERROR)$ ]]; then
                ...
                echo "Error: invalid value for --http-log-level: $2"
                echo "Allowed values: DEBUG, INFO, WARNING, ERROR"
                exit 1
            fi
            ;;
        ...
        *)
            ...
            echo "Usage: $0 --videoreg-project-home <path> [--log-level <level>] [--http-log-level <level>] [--env <env>]"
            exit 1
            ;;
    esac
done
...
if [ -z "$VIDEOREG_PROJECT_HOME" ]; then
    ...
    echo "Usage: $0 --videoreg-project-home <path> [--log-level <level>] [--http-log-level <level>]"
    exit 1
fi
...
PYTHON_CMD="\"$VIDEOREG_PROJECT_HOME/.venv/bin/python3\" \"$VIDEOREG_PROJECT_HOME/run.py\" --service=\"vrg-bot\" --videoreg-project-home=\"$VIDEOREG_PROJECT_HOME\" --log-level=\"$LOG_LEVEL\" --http-log-level=\"$HTTP_LOG_LEVEL\""
if [[ -n "$ENV" ]]; then
