  ...
  template: `
    <div>
      <div class="page-header">
        <h1 class="page-title">{{ $t('camera.media_feed.title') }}</h1>
...
<div v-if="error" class="alert alert-error">{{ error }}</div>
  ...
  methods: {
    ...
    async loadAll() {
      ...
      this.expandedGroups = {};
      try {
        const response = await fetch('/api/camera/list_media?all=true', { credentials: 'same-origin' });
        const data = await response.json();
        if (!response.ok) {
          this.error = data.error || this.$t('camera.media_feed.error_load');
          return;
        }
