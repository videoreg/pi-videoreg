const SmsInboxComponent = {
  ...
  template: `
    <div>
      <div class="page-header">
        <h1 class="page-title">{{ $t('sms.sms_inbox.title') }}</h1>
  ...
  methods: {
    async load() {
      ...
      this.loading = true;
      try {
        const response = await fetch('/api/sms/list', { credentials: 'same-origin' });
        const data = await response.json();
        if (!response.ok) {
          this.error = data.error || this.$t('sms.sms_inbox.error_load');
          return;
        }
    ...
    async deleteMsg(msg) {
      if (!confirm(this.$t('sms.sms_inbox.confirm_delete', { number: msg.number }))) return;
      try {
        const response = await fetch('/api/sms/delete', {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify({ filename: msg.filename })
        });
        if (!response.ok) {
          const data = await response.json();
          this.error = data.error || this.$t('sms.sms_inbox.error_delete');
          return;
        }
