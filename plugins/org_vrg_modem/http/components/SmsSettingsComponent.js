...
const SmsSettingsComponent = {
  ...
  template: `
    <div>
      <div class="page-header">
        ...
        <h1 class="page-title">{{ $t('sms.sms.title') }}</h1>
      </div>
      ...
      <div class="content-section">
        ...
        <div v-if="activeTab === 'users'">
          ...
          <div class="info-block" style="margin-top: var(--spacing-xl);">
            <div class="section-title">{{ $t('sms.sms.phone_format_title') }}</div>

            <ul style="margin-left: var(--spacing-lg); color: var(--color-text-secondary);">
              <li>{{ $t('sms.sms.phone_format_hint1') }}</li>
              <li>{{ $t('sms.sms.phone_format_example') }}: <code style="background: var(--color-bg-tertiary); padding: 2px 6px; border-radius: var(--radius-sm);">+79001234567</code></li>
              <li>{{ $t('sms.sms.phone_format_hint2') }}</li>
            </ul>
          </div>
        </div>
      </div>
  ...
  computed: {
    tabs() {
      return [
        { value: 'users', label: this.$t('sms.sms.tab_users') }
      ];
    }
  },
    ...
    async loadUsers() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          this.error = data.error || this.$t('sms.sms.error_load_users');
          return;
        }
        const data = await response.json();
    ...
    async saveUsers() {
      ...
      try {
        for (const user of this.users) {
          ...
          if (!response.ok) {
            const data = await response.json();
            this.error = data.error || this.$t('sms.sms.error_save_user', { username: user.username });
            return;
          }
        }
        this.success = this.$t('sms.sms.users_saved');
      } catch (err) {
        this.error = this.$t('http.common.error_server');
