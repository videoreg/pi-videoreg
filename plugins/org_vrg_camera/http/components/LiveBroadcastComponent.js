  ...
  template: `
    <div>
      <div class="page-header">
        <h1 class="page-title">{{ $t('camera.live.title') }}</h1>
        <button
          class="btn"
          ...
          :disabled="busy"
        >
          {{ streaming ? $t('camera.live.stop') : $t('camera.live.start') }}
        </button>
      </div>
  ...
  methods: {
    ...
    async start() {
      ...
      try {
        ...
        if (!r.ok) {
          this.error = this.$t('camera.live.error_start');
          await this.stop();
          return;
        }
        this.streaming = true;
    ...
    _waitForPlaylist(url) {
      ...
      const tick = async () => {
        ...
        if (tries < 30) {
          ...
          this.error = this.$t('camera.live.error_no_stream');
          await this.stop();
        }
      };
      tick();
