...
class PiSugar:
  ...
  async def get_charging_status(self) -> int:
    result = await self._exec_pisugar_command(["get_charging_status"])
    if result.returncode == 0:
      return 1 if result.stdout.rstrip() == "true" else -1
    else:
      return 0

  async def get_charging_status_slow_but_safe(self) -> int:
    """
    Get charging status assuming PiSugar can return wrong status.
    If get "-1" first, then wait 1 sec and doublecheck.
    """

    is_charging = await self.get_charging_status()
    if is_charging == -1:
