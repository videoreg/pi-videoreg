  ...
  template: `
    <div>
      <div class="page-header">
        <h1 class="page-title">{{ $t('stat.stat.title') }}</h1>
      ...
      <div v-if="everLoaded">
        ...
        <div v-if="activeTab === 'cpu'" class="info-block">
          <div style="margin-bottom: var(--spacing-md);">
            <span style="font-weight: 600;">{{ $t('stat.stat.cpu_temp_title') }}</span>
        ...
        <div v-if="activeTab === 'pisugar'" class="info-block">
          <div style="margin-bottom: var(--spacing-md);">
            <span style="font-weight: 600;">{{ $t('stat.stat.pisugar_title') }}</span>
            <span v-if="pisugarData.length > 0" style="margin-left: var(--spacing-md); color: var(--color-text-secondary); font-size: 0.875rem;">
              {{ pisugarDate }} &middot; {{ pisugarData.length }} {{ $t('stat.stat.points') }}
            </span>
          </div>
        ...
        <div v-if="activeTab === 'traffic'">
          <div class="info-block" style="margin-bottom: var(--spacing-lg);">
            ...
            <multi-line-chart :series="downloadSeries" :unit="$t('stat.stat.traffic_unit')"></multi-line-chart>
          </div>
          <div class="info-block">
            <div style="margin-bottom: var(--spacing-md);">
              <span style="font-weight: 600;">Upload</span>
            </div>
            <multi-line-chart :series="uploadSeries" :unit="$t('stat.stat.traffic_unit')"></multi-line-chart>
          </div>
        </div>
  ...
  computed: {
    tabList() {
      return [
        ...
        { value: 'traffic', label: this.$t('stat.stat.tab_traffic') }
      ];
    },
  ...
  methods: {
    ...
    async loadCpuPisugarData() {
      ...
      if (!tempRes.ok || !pisugarRes.ok) {
        this.error = this.$t('stat.stat.error_load');
        return;
      }
    ...
    async loadTrafficData() {
      ...
      if (!res.ok) {
        this.error = this.$t('stat.stat.error_traffic');
        return;
      }
