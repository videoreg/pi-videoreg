...
Обратите внимание, что плата Rasbperry Pi сама по себе не умеет сохранять время между перезагрузками. Есть 2 способа автоматического определения времени после перезагрузки:
...
2. Текущее время сохраняется в RTC в PiSugar 3 (также обновляется после NTP-синхронизации) и при следующем запуске даже подключения к Интернету время будет актуальным.
...
3. В чате с ботом выполните команду `/start` — она проинициализрует меню бота.
...
Если вы используете USB-модем SIM7600, то вы можете настроить выпонение команд и переадресацию SMS ваш мобильный телефон. Для этого укажите свой номер телефона в Настройках `SMS`.

Функции SMS на модеме A7670 пока не поддерживаются.
...
Но в целом, перевод текущей реализации на компилируемый язык выглядит вполне разумным развитием проекта. Языки `C/С++` видятся наиболее перспективными из-за возможности переиспольнования логики в реализации под ESP32.

> [!TIP]
> Важно подчеркнуть, что основная задача, запись видео, по-умолчанию реализована на C++ и испльзует hardware-кодеки. А именно, запись видео и фото происходят через доработанную версию [rpicam-apps](https://github.com/videoreg/rpicam-apps).
...
Вы можете использовать основные функции видеорегистратора: запись видео и парковочный режим. Доступ к Web UI вы можете получать через точку доступа WiFi (Access Point), которую создаёт устройство.
...
Протестирована работа устройства с модемами SIM7600 и A7670 (см. [подробнее про A7670.md](A7670.md)).
...
Пожалуйста, не используйте устройство при температуре окрудающей среды выше `35°C`.

### 🏛️ Архитектура

Основные компоненты: `systemd-сервисы` и `плагины`.

**Systemd-сервисы** с префиксом `vrg-*` — основные процессы, в рамках которых выполняются один или несоклько плагинов.
...
- Возможность создавать сервисы реализованные на разных технологиях. Например, можно реализовать сервис на `C/C++` который будет взаимодейстовать с другими сервисами на `Python` через event bus.
...
**Проблема отключения модема в парковочном режиме**: при использовании PiSugar UPS, USB-модем теряет питание вместе с RPi. Включается он так же безусловно вместе c RPi. И если мы говорим о режиме периодического мониторинга на парковке, то у такого поведения есть следующие недостатки:
...
**Унифицировать взаимодействие с USB-модемами типа QMI и остальными:** как было упомянуто выше, в отличие от модема SIM7600 модем A7670 требует ручной настройки и не поддерживается ModemManager (через который мы получаем доступ к СМС и GPS). В качестве решения последней проблемы, можно отказаться от использования ModemManager и получать данные (СМС и локацию) напрямую из `/dev/ttyUSB2` & `/dev/ttyUSB3` соответственно.

**Повысить безопасность связанную с начальным запуском:** при первом запуске устройства автоматически создается пользователь `admin` с преднастроенным паролем, а так же WiFi Access Point `videoreg`.
...
**Создать плагин выгрузки контента в удаленное сетевое хранилище:** выгружать фото и видео, уловно, на домашний NAS.
...
**Создать движок уведомлений пользователя:** рассмторим проблему на примере переадресации входящих СМС в Telegram. Если по каким-то причинам отправка сообщения в Telegram не была выполнена (например, были проблемы с сетью), то отправка не повторится. Похожее поведение сейчас будет во всех случаях.
