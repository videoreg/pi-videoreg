...
async def get_tracks_keyboard(videoreg: Videoreg, logger: Logger, page: int) -> TracksKeyboardData:
  per_page = 6
  offset = per_page * (page - 1)
  dir = str(videoreg.gps_path())

  all_files = [f for f in os.listdir(dir) if os.path.isfile(os.path.join(dir, f))]
  all_files.sort(reverse=True)
  all_files_count = len(all_files)

  result = TracksKeyboardData()
      ...
      buttons_row.append(
        {
          ...
          "callback_data": f"command__gps__list_tracks__{x5_page}",
        }
      )

    buttons_row.append(
      {"text": "⬅️ Earlier", "callback_data": f"command__gps__list_tracks__{next_page}"}
    )
  if page > 1:
    prev_page = page - 1
    buttons_row.append(
      {"text": "Later ➡️", "callback_data": f"command__gps__list_tracks__{prev_page}"}
    )
